    ...
    <div class="flex flex-col gap-6">
        ...
        <div>
            ...
            <div class="flex flex-col gap-3 mt-3">
                <select wire:model.live="state_id" class="w-full appearance-none bg-white/80 border border-gray-300/60 rounded px-4 py-3 text-xs cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500">
                    <option value="">All States</option>
                    @foreach($this->states as $state)
                        <option value="{{ $state->id }}">{{ $state->name }}</option>
                    @endforeach
                </select>
                <select wire:model.live="city_id" @if(!$state_id) disabled @endif class="w-full appearance-none bg-white/80 border border-gray-300/60 rounded px-4 py-3 text-xs cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 disabled:opacity-50">
