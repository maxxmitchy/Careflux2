        ...
        <div class="grid lg:grid-cols-2 bg-white rounded shadow-2xl shadow-emerald-200/50 overflow-hidden">
            ...
            <div class="hidden lg:block relative p-8 bg-gray-900">
                ...
                <div class="absolute inset-0 opacity-20">
                    <svg class="absolute top-0 left-0 w-64 h-64 text-emerald-500 transform -translate-x-1/2 -translate-y-1/2"
                        fill="currentColor" viewBox="0 0 200 200">
                        <path d="M 100, 100 m -75, 0 a 75,75 0 1,0 150,0 a 75,75 0 1,0 -150,0" />
                    </svg>
                    <svg class="absolute bottom-0 right-0 w-72 h-72 text-emerald-500 transform translate-x-1/2 translate-y-1/2"
                        fill="currentColor" viewBox="0 0 200 200">
                        <path d="M 100, 100 m -75, 0 a 75,75 0 1,0 150,0 a 75,75 0 1,0 -150,0" />
                    </svg>
                </div>
            ...
            <div x-cloak class="p-6 sm:p-10" x-data="{ role: @entangle('role').live }">
                ...
                <div class="grid grid-cols-2 gap-2 p-1 bg-gray-100 rounded mt-6 text-sm font-medium">
                    ...
                    <button @click="role = 'patient'"
                        ...
                        <!-- Icon -->
                        <svg xmlns="http://www.w3.org/2000/svg" class="flex-shrink-0 h-4 w-4" fill="none"
                ...
                <div class="relative h-12 mt-4">
                    <div x-show="role === 'patient'" x-transition:enter="transition ease-out duration-300"
                        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                        class="absolute inset-0">
                        <p class="text-xs text-gray-600">Create your free patient account to manage prescriptions, track
                            orders, and connect with your personal pharmacist.</p>
                    </div>
                    <div x-show="role === 'technician'" x-cloak x-transition:enter="transition ease-out duration-300"
                        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                        class="absolute inset-0">
                        <p class="text-xs text-gray-600">Join your pharmacy's team on Careflux to manage inventory and
                            fulfill orders. Your account requires approval.</p>
                    </div>
                </div>
                ...
                <form wire:submit.prevent="register" class="mt-4 space-y-4">
                    ...
                    <div>
                        <input wire:model="name" type="text" placeholder="Full Name"
                            class="w-full p-3 border border-gray-300 text-sm rounded focus:outline-emerald-600">
                        @error('name')
                    ...
                    <div>
                        <input wire:model="phone" type="tel" placeholder="Phone Number"
                            class="w-full p-3 border border-gray-300 text-sm rounded focus:outline-emerald-600">
                        @error('phone')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div x-show="role === 'technician'" x-collapse class="relative">
                        <select wire:model="pharmacy_id"
                            ...
                            @foreach ($pharmacies as $pharmacy)
                                <option class="text-gray-600" value="{{ $pharmacy->id }}">{{ $pharmacy->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <button type="submit"
                        class="w-full bg-emerald-600 text-white py-3 rounded text-sm font-semibold hover:bg-emerald-700 focus:ring-2 focus:ring-emerald-500">
                        Create Account
                    </button>
                </form>
                ...
                <div class="mt-4 text-center text-xs text-gray-600">
                    ...
                    <a x-show="role === 'patient'" href="{{ route('filament.patient.auth.login') }}"
                        class="text-emerald-600 font-semibold hover:underline" wire:navigate>
                        Log in
                    </a>
                    <a x-show="role === 'technician'" href="{{ route('filament.technician.auth.login') }}"
                        class="text-emerald-600 font-semibold hover:underline" wire:navigate x-cloak>
                        Log in
                    </a>
                </div>
            </div>
